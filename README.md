...
Gemini 워터마크는 우측 하단에 찍히는 매끄러운 4방향 스파클(반짝임) 모양입니다.
공개된 분석 글은 해상도별 고정 픽셀 크기 표를 제시했지만, 실제 Gemini 이미지
샘플로 직접 검증해보니 그 수치와 달랐습니다 — 그래서 이 프로젝트는 고정된
표 대신, **위치 비율 기반 탐지**를 사용합니다.

- 1차 탐색: 실제 샘플에서 측정한 위치(하단·우측 모서리에서 각각 이미지 짧은
  변의 약 4.5% 안쪽)를 중심으로 좁은 범위만 탐색합니다. 배경(손, 가구 모서리
  등)이 스파클보다 더 그럴듯하게 매칭되는 오탐을 줄이기 위해 일부러 좁게
  잡았습니다.
- 2차 탐색(폴백): 1차에서 확신 있는 매칭을 못 찾으면 우측 하단 모서리
  16%×16% 영역 전체를 더 엄격한 기준으로 한 번 더 탐색합니다.
- 두 단계 모두 여러 후보 크기(이미지 짧은 변의 1.6~5.0%)로 그레이스케일
  그래디언트(Sobel) 기반 템플릿 매칭을 수행해 신뢰도 점수를 계산합니다.
  점수가 임계값 미만이면 "탐지 실패"로 보고 `failed/`로 보냅니다.

기본 템플릿은 근사치로 그린 스파클 모양(4방향 아스트로이드 곡선)입니다.
**정확도를 높이려면** 본인이 가진 Gemini 이미지 중 하나에서 워터마크 부분만
정사각형으로 크롭해 `models/watermark_template.png`로 저장해두세요. (실제
Google 로고 크롭이라 저장소에는 커밋되지 않도록 `.gitignore`에 포함되어
있습니다 — 본인 로컬 파일을 매칭용 템플릿으로만 사용하며 배포되거나 외부로
전송되지 않습니다.)

실제 20장 샘플로 테스트한 결과 약 75%(15/20)에서 정확히 탐지되었고, 나머지는
흰 배경 위에 흰 워터마크처럼 육안으로도 거의 안 보이는 경우였습니다 — 이런
경우는 억지로 처리하지 않고 `failed/`로 안전하게 보냅니다.
...
워터마크가 알파 블렌딩으로 합성된다는 사실과 대략적인 탐지 아이디어는 아래
공개 분석 글에서 참고했습니다 (코드는 가져오지 않았으며, 정확한 픽셀 수치는
이 글의 표와 다르다는 것을 실제 샘플로 확인해 직접 재측정했습니다):
