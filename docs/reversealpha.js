/*
 * Reverse alpha-blend watermark removal.
 *
 * Gemini's visible sparkle watermark is alpha-composited onto the image:
 *   observed = original*(1-alpha) + logoColor*alpha
 * Since the watermark's shape/alpha map and color (solid white) are known,
 * this inverts that formula directly:
 *   original = (observed - alpha*logoColor) / (1-alpha)
 * This is exact math, not a generative guess, so it runs with zero AI
 * model, zero download, and is essentially instant.
 *
 * Two things this file deliberately does NOT do, based on real testing:
 *   - Trust a single hardcoded (size, margin) tier. Empirically, real
 *     Gemini output did not match any of the publicly documented tiers
 *     well (confidence <0.31 against all of them) — free-searching the
 *     corner region against the real alpha shape found the true position
 *     at far higher confidence (0.79-0.91) at a geometry none of those
 *     tiers predicted. Gemini's exact placement isn't assumed stable.
 *   - Assume alpha=1.0 gain. The reference alpha map's peak alpha
 *     over-subtracts on real (JPEG, i.e. lossy) Gemini downloads — the
 *     source repos document this exact caveat (works best on lossless
 *     PNG). An empirically-tuned GAIN factor is applied to correct for it.
 *
 * Alpha map sourced from PlayerYK/GeminiWatermarkRemover (MIT License,
 * Copyright (c) 2025 PlayerYK) — https://github.com/PlayerYK/GeminiWatermarkRemover
 * See README.md "Attribution & Sources".
 */

const ALPHA_MAP_PNG_BASE64_96 = "";

const LOGO_VALUE = 255;      // the watermark is solid white
const MAX_ALPHA = 0.99;      // clamp so 1-alpha never hits 0
const ALPHA_THRESHOLD = 0.02;
const GAIN = 0.6;            // empirically calibrated against real JPEG samples — see comment above
const SEARCH_STRIDE = 2;

// Candidate sizes as a fraction of min(width, height), searched within the
// bottom-right corner. Wide enough to cover both known tiers (48px/96px on
// a 1024px image is ~4.7%/9.4%) plus headroom either side.
const SIZE_FRACTIONS = [0.03, 0.035, 0.04, 0.045, 0.05, 0.055, 0.06, 0.07, 0.08, 0.09, 0.1];
const CORNER_FRACTION = 0.22;
const CONFIDENCE_THRESHOLD = 0.5;

let _baseAlphaImg = null;
async function _loadBaseAlphaImage() {
  if (_baseAlphaImg) return _baseAlphaImg;
  _baseAlphaImg = await new Promise((resolve, reject) => {
    const el = new Image();
    el.onload = () => resolve(el);
    el.onerror = reject;
    el.src = `data:image/png;base64,${ALPHA_MAP_PNG_BASE64_96}`;
  });
  return _baseAlphaImg;
}

const _resizedAlphaCache = {};
async function getAlphaMapAtSize(size) {
  if (_resizedAlphaCache[size]) return _resizedAlphaCache[size];
  const img = await _loadBaseAlphaImage();
  const canvas = document.createElement("canvas");
  canvas.width = size; canvas.height = size;
  const ctx = canvas.getContext("2d");
  ctx.imageSmoothingEnabled = true;
  ctx.imageSmoothingQuality = "high";
  ctx.drawImage(img, 0, 0, size, size);
  const data = ctx.getImageData(0, 0, size, size).data;
  const alpha = new Float32Array(size * size);
  for (let p = 0, i = 0; i < data.length; i += 4, p++) {
    alpha[p] = Math.max(data[i], data[i + 1], data[i + 2]) / 255;
  }
  _resizedAlphaCache[size] = alpha;
  return alpha;
}

function grayFromImageData(imageData) {
  const { data, width, height } = imageData;
  const gray = new Float32Array(width * height);
  for (let i = 0, p = 0; i < data.length; i += 4, p++) {
    gray[p] = (data[i] + data[i + 1] + data[i + 2]) / 3;
  }
  return gray;
}

function correlationAt(gray, grayW, alphaMap, size, x0, y0) {
  let sumA = 0, sumB = 0, sumAB = 0, sumA2 = 0, sumB2 = 0;
  const n = size * size;
  for (let y = 0; y < size; y++) {
    const rowBase = (y0 + y) * grayW + x0;
    const aRowBase = y * size;
    for (let x = 0; x < size; x++) {
      const b = gray[rowBase + x] / 255;
      const a = alphaMap[aRowBase + x];
      sumA += a; sumB += b; sumAB += a * b; sumA2 += a * a; sumB2 += b * b;
    }
  }
  const meanA = sumA / n, meanB = sumB / n;
  const cov = sumAB / n - meanA * meanB;
  const varA = sumA2 / n - meanA * meanA;
  const varB = sumB2 / n - meanB * meanB;
  const denom = Math.sqrt(varA * varB);
  return denom > 1e-6 ? Math.abs(cov / denom) : 0;
}

const _yield = () => new Promise((r) => setTimeout(r, 0));

/**
 * Free-searches the bottom-right corner for the watermark using the real
 * alpha shape as the matching template (far more discriminative than a
 * synthetic approximation). Returns {x, y, size, confidence} or null.
 */
async function findWatermark(imageData) {
  const { width, height } = imageData;
  const gray = grayFromImageData(imageData);
  const minDim = Math.min(width, height);
  const roi = Math.round(minDim * CORNER_FRACTION);
  const rx0 = Math.max(0, width - roi), ry0 = Math.max(0, height - roi);

  let best = null;
  for (const frac of SIZE_FRACTIONS) {
    const size = Math.max(16, Math.round(minDim * frac));
    if (size > roi) continue;
    const alphaMap = await getAlphaMapAtSize(size);
    for (let y = ry0; y <= height - size; y += SEARCH_STRIDE) {
      for (let x = rx0; x <= width - size; x += SEARCH_STRIDE) {
        const conf = correlationAt(gray, width, alphaMap, size, x, y);
        if (!best || conf > best.confidence) best = { x, y, size, confidence: conf };
      }
    }
    await _yield();
  }
  if (!best || best.confidence < CONFIDENCE_THRESHOLD) return null;
  return best;
}

/** Reverses the alpha blend in place within the given box. */
async function applyReverseAlphaBlend(imageData, x0, y0, size) {
  const alphaMap = await getAlphaMapAtSize(size);
  const { data, width } = imageData;
  for (let y = 0; y < size; y++) {
    for (let x = 0; x < size; x++) {
      const rawAlpha = alphaMap[y * size + x];
      if (rawAlpha < ALPHA_THRESHOLD) continue;
      const alpha = Math.min(rawAlpha * GAIN, MAX_ALPHA);
      const oneMinusAlpha = 1 - alpha;
      const idx = ((y0 + y) * width + (x0 + x)) * 4;
      for (let c = 0; c < 3; c++) {
        const observed = data[idx + c];
        const original = (observed - alpha * LOGO_VALUE) / oneMinusAlpha;
        data[idx + c] = Math.max(0, Math.min(255, Math.round(original)));
      }
    }
  }
}

/**
 * Full pipeline on an ImageData (mutated in place on success).
 * Returns {applied, box:[x1,y1,x2,y2], confidence} — applied=false means
 * nothing was confidently found and no pixels were touched.
 */
async function removeWatermarkByReverseAlphaBlend(imageData) {
  const found = await findWatermark(imageData);
  if (!found) return { applied: false, box: null, confidence: 0 };
  await applyReverseAlphaBlend(imageData, found.x, found.y, found.size);
  return {
    applied: true,
    box: [found.x, found.y, found.x + found.size, found.y + found.size],
    confidence: found.confidence,
  };
}

window.reverseAlphaModule = {
  findWatermark,
  applyReverseAlphaBlend,
  removeWatermarkByReverseAlphaBlend,
  getAlphaMapAtSize,
};
